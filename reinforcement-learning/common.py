...
def main_testing(sess, env, graph_map, config):
  ...
  while not done:
    ...
    time.sleep(config.delta)

def main_training(sess, env, graph_map, config, memory):
  epoch_memory = []
  episode_memory = []
...
def main(env, config, memory, test=False, load=None, save=None, log=None):
  ...
  with tf.Session(config=tf_config) as sess:
    ...
    try:
      if test:
        while True:
          main_testing(sess, env, graph_map, config)
      else:
        train_step = 0
        smooth_score = None

        while True:
          score, loss = main_training(sess, env, graph_map, config, memory)
